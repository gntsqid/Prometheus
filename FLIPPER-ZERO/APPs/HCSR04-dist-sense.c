...
float hc_sr04_us_to_m(uint32_t us) {
    const float speed_sound_m_per_s = 344.04f; // Speed of sound (m/s)
    const float time_s = us / 1e6f;
    const float total_dist = time_s * speed_sound_m_per_s;
    return total_dist / 2.0f; // Divided by 2 for one-way distance
}
...
int32_t ultrasonic_distance_app() {
